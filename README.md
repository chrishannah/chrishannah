I am a:
- Senior Software Engineer at Ant Group/WorldFirst working backend systems for the World Card product.
- iOS/macOS developer. My current apps are [Text Shot](https://chrishannah.me/text-shot/) and [Text Case](https://textcase.app/).
- Contributor to free, open-source software.
- Writer/blogger at [chrishannah.me](https://chrishannah.me).

I'm also currently open for new roles. If you're interested, [check out my online cv](https://chrishannah.me/cv).

## 🚀 Current Projects

### Active Development
- **Minibase** - Shared framework for Miniship & Miniroll SaaS applications
- **Miniroll** - Blogroll management tool with RSS aggregation
- **Miniship** - In development
- **[Text Case](https://textcase.app)** - Main Text Case project
  - [Text Case CLI](https://github.com/chrishannah/textcase-cli) - Command line version
  - [Text Case Web](https://github.com/chrishannah/Text-Case-Web) - Web version
  - [textcase.app](https://github.com/chrishannah/textcase-website) - Marketing website
  - [Landing Page](https://github.com/chrishannah/text-case-landing)
- **Text Shot** - Screenshot text extraction
- **[Pokeguessr](https://github.com/chrishannah/pokeguessr)** - CLI game to guess all 151 Pokémon

### Open Source Tools
Check out my [scripts collection](https://github.com/chrishannah/scripts) for various utility scripts.

## 📫 Contact

- <a rel="me" href="https://twitter.com/chrishannah">Twitter</a>
- <a rel="me" href="https://fosstodon.org/@chrishannah">Mastodon</a>
- <a href="mailto:me@chrishannah.me">Email</a>

---

[![Stats](https://awesome-github-stats.azurewebsites.net/user-stats/chrishannah?cardType=octocat&theme=tokyonight&fontFamily=JetBrains%20Mono&preferLogin=true)](https://git.io/awesome-stats-card)

*Profile auto-updated weekly*
